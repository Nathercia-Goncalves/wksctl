...
func TestService(t *testing.T) {
	...
	startingState, err := service.QueryState(context.TODO(), r)
	...
	_, err = service.Apply(context.TODO(), r, startingDiff)
	assert.NoError(t, err)

	// Verify the state is correctly applied.
	realizedState, err := service.QueryState(context.TODO(), r)
	assert.NoError(t, err)
	...
	r.ResetOperations()
	_, err = service.Apply(context.TODO(), r, realizedDiff)
	assert.NoError(t, err)
	assert.Equal(t, 0, len(r.Operations()))

	// Undo the install.
	err = service.Undo(context.TODO(), r, realizedState)
	assert.NoError(t, err)
	undoState, err := service.QueryState(context.TODO(), r)
	assert.NoError(t, err)
