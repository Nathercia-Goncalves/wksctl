...
func TestRPM(t *testing.T) {
	...
	// Install make.
	_, err := p.Apply(context.TODO(), r, emptyDiff)
	assert.NoError(t, err)

	// Verify make is installed.
	installedState, err := p.QueryState(context.TODO(), r)
	assert.NoError(t, err)
	...
	_, err = p.Apply(context.TODO(), r, installedDiff)
	assert.NoError(t, err)
	assert.Equal(t, 0, len(r.Operations()))

	// Undo the install.
	err = p.Undo(context.TODO(), r, installedState)
	assert.NoError(t, err)
	testutils.AssertEmptyState(t, p, r)
}
