...
func TestFile(t *testing.T) {
	...
	_, err := file.Apply(context.TODO(), r, emptyDiff)
	assert.NoError(t, err)

	remoteContent, err := r.RunCommand(context.TODO(), fmt.Sprintf("cat %q", file.Destination), nil)
	assert.NoError(t, err)
	...
	realizedState, err := file.QueryState(context.TODO(), r)
	assert.NoError(t, err)
	assert.Equal(t, realizedState, file.State())

	// Undo and check the file isn't there.
	err = file.Undo(context.TODO(), r, realizedState)
	assert.NoError(t, err)
